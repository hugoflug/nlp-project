...
def main():
    ...
    #spotter = GoldSpotter()
    #pruner = DumbPruner()
    entity_linker = EntityLinker(spotter=spotter, similarity=sim, annotator=MultiAnnotator(None, wiki_annotator, bing_annotator))
    ...
    #pruner = DumbPruner()
    entity_linker = EntityLinker(spotter=spotter, similarity=sim, annotator=MultiAnnotator(None, wiki_annotator, bing_annotator))
    evaluator.evaluate(entity_linker.annotate, "query-data-train-set.xml")
    sim.save_cache() # save if we have any new similarities to cache on file
    bing_annotator.save_cache()
    wiki_annotator.save_cache()
    spotter.save_cache()
